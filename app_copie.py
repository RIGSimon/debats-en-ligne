    ...
    def next_step(self, choice):
        ...
        if ((self.index + 2 >= len(self.order)) and not self.stop):
            ...
            arg_button = tk.Button(self.root, 
                                ...
                                command=lambda: analyse_window(self.score, self.weighted_score, meanTab, self.pour, self.contre, self.transitions_results, self.root, self.debate_graph.nodes[self.debate_graph.main_arg].get("text", self.debate_graph.main_arg), self.strategy, myUsername),
                                wraplength=500, 
                                justify="left")
